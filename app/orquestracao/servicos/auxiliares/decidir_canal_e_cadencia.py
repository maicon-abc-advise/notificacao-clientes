...
def _bounce_hard_email(estado: str) -> bool:
    return estado in (
        EngajamentoEstado.EMAIL_BOUNCE_HARD_SEM_SMS.value,
        EngajamentoEstado.EMAIL_BOUNCE_HARD_SMS_FILA.value,
    )


def _sms_problematico(estado: str) -> bool:
    return estado in (
        EngajamentoEstado.SMS_FALHA_NUMERO.value,
        EngajamentoEstado.SMS_FALHA_LIMITE.value,
    )
...
def decidir_canal_e_cadencia(
    *,
    ...
    email_ok = bool(email_efetivo) and recebe_email and not _bounce_hard_email(engajamento_estado)

    if email_ok:
        return DecisaoCanal("email", CodigoTipoTemplate.APARECEU_BUSCA, "e-mail disponível e permitido")

    if telefone_efetivo and not _sms_problematico(engajamento_estado):
        ...
        elif bool(email_efetivo) and _bounce_hard_email(engajamento_estado):
